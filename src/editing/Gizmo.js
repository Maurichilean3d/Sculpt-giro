...
class Gizmo {
  ...
  constructor(main) {
    ...
    // stable rotation interaction (mouse ray -> plane)
    this._rotStartVec = vec3.create();
    this._rotCurVec = vec3.create();
    this._rotAxisWorld = vec3.create();
    this._rotPlaneOrigin = vec3.create();
  ...
  _initRotate() {
    ...
    // Primitives.createTorus produces a torus whose normal points to +Y (ring lies in XZ plane).
    // We want rotation rings that follow the standard convention:
    // - X ring: lies in YZ plane (normal +X)
    // - Y ring: lies in XZ plane (normal +Y) [identity]
  ...
  _startRotateEdit() {
    // Stable rotation:
    // - Compute the axis in WORLD
    // - Intersect the mouse ray with the plane through the gizmo center
    //   whose normal is the rotation axis
    // - Store a start vector on that plane
    var main = this._main;
    var camera = main.getCamera();

    // plane origin (world)
    this._computeCenterGizmo(this._rotPlaneOrigin);

    // axis (world)
    var nbAxis = this._selected._nbAxis;
    if (this._spaceMode === SPACE_WORLD) {
      vec3.set(this._rotAxisWorld, 0.0, 0.0, 0.0);
      this._rotAxisWorld[nbAxis] = 1.0;
    } else {
      // take the axis from the space matrix (column nbAxis)
      this._rotAxisWorld[0] = this._spaceMatrix[nbAxis * 4];
      this._rotAxisWorld[1] = this._spaceMatrix[nbAxis * 4 + 1];
      this._rotAxisWorld[2] = this._spaceMatrix[nbAxis * 4 + 2];
      vec3.normalize(this._rotAxisWorld, this._rotAxisWorld);
    }

    // ray in world
    var s1 = camera.unproject(main._mouseX, main._mouseY, 0.0);
    var s2 = camera.unproject(main._mouseX, main._mouseY, 0.1);

    // intersection with plane
    Geometry.intersectLinePlane(s1, s2, this._rotPlaneOrigin, this._rotAxisWorld, this._rotStartVec);
    vec3.sub(this._rotStartVec, this._rotStartVec, this._rotPlaneOrigin);
    // project to plane (numerical safety)
    vec3.scaleAndAdd(this._rotStartVec, this._rotStartVec, this._rotAxisWorld, -vec3.dot(this._rotStartVec, this._rotAxisWorld));
    if (vec3.len(this._rotStartVec) === 0.0) {
      // fallback: choose any perpendicular vector
      vec3.copy(this._rotStartVec, Geometry.getPerpendicularVector(this._rotAxisWorld));
    }
    vec3.normalize(this._rotStartVec, this._rotStartVec);
  }
  ...
  _updateRotateEdit() {
    ...
    // intersection with plane
    Geometry.intersectLinePlane(s1, s2, this._rotPlaneOrigin, this._rotAxisWorld, this._rotCurVec);
    vec3.sub(this._rotCurVec, this._rotCurVec, this._rotPlaneOrigin);
    vec3.scaleAndAdd(this._rotCurVec, this._rotCurVec, this._rotAxisWorld, -vec3.dot(this._rotCurVec, this._rotAxisWorld));
    if (vec3.len(this._rotCurVec) === 0.0) return;
    vec3.normalize(this._rotCurVec, this._rotCurVec);

    // signed angle between start and current vectors around the axis
    var cross = vec3.cross(vec3.create(), this._rotStartVec, this._rotCurVec);
    var sin = vec3.dot(cross, this._rotAxisWorld);
    var cos = vec3.dot(this._rotStartVec, this._rotCurVec);
    var angle = Math.atan2(sin, cos);

    // helper line (projected 2D) just for feedback
    var p0 = camera.project(vec3.add(vec3.create(), this._rotPlaneOrigin, this._rotStartVec));
    var p1 = camera.project(vec3.add(vec3.create(), this._rotPlaneOrigin, this._rotCurVec));
    this._updateLineHelper(p0[0], p0[1], p1[0], p1[1]);

    var meshes = this._main.getSelectedMeshes();
    for (var i = 0; i < meshes.length; ++i) {
      var mrot = meshes[i].getEditMatrix();
      mat4.identity(mrot);
      mat4.fromRotation(mrot, -angle, this._rotAxisWorld);
