...
class Transform extends SculptBase {
  ...
  applyEditMatrix(iVerts) {
    ...
    // IMPORTANT:
    // The gizmo (src/editing/Gizmo.js) already bakes the chosen space (World/Local/Normal)
    // into the edit matrix. Applying another space-conjugation here would permute axes
    // (e.g. X->Y, Y->Z, Z->X). So we always apply the edit matrix as-is.
    var emApply = em;
