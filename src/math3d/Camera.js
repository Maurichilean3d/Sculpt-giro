...
class Camera {
  ...
  constructor(main) {
    ...
    // canonical UP vector (Y-up by default, Z-up optional via ?up=z)
    this._up = getUpVector(vec3.create());
  ...
  updateView() {
    ...
    // refresh up vector in case URL options changed (rare, but safe)
    getUpVector(this._up);
    mat4.lookAt(view, _TMP_VEC3_2, center, this._up);
